...
class StressQuestionnaireApp:
    def __init__(self, master):
        self.master = master
        self.master.title("Stress Diagnosis Questionnaire")
        self.questions = [
        ...
        self.label = tk.Label(master, text=self.questions[self.question_index])
        self.label.pack()

        self.response_var = tk.IntVar()
        self.response_var.set(0)
        for i in range(1, 6):
            tk.Radiobutton(master, text=str(i), variable=self.response_var, value=i).pack()

        self.next_button = tk.Button(master, text="Next", command=self.next_question)
        self.next_button.pack()

    def next_question(self):
        self.responses.append(self.response_var.get())
    ...
    def submit_responses(self):
        ...
        messagebox.showinfo("Stress Level", f"Your stress level is: {stress_level}")

        if stress_level != "Low stress":
            tip = random.choice(self.stress_tips)
            messagebox.showinfo("Stress Tip", f"Tip: {tip['tip']}\n\nTo do this: {tip['example']}")
        else:
            messagebox.showinfo("Congratulations", "Congratulations, Keep up the good work!")
...
def main():
    root = tk.Tk()
    app = StressQuestionnaireApp(root)
    root.mainloop()
